...
class MS_Model_LSTM:
    ...
    def LSTM_step_backward(self,da_next,dc_next,cache_t,parameters,gradients):
        ...
        zc = np.dot(Wca,a_prev)+np.dot(Wcx,x_t)+bc
        ...
        dc_t = dc_next + da_t*Gamma_o*self.activation_derivative(c_t,activation="relu")
        
        dZf = dc_t*c_prev*Gamma_f*(1-Gamma_f)
        dZu = dc_t*c_st*Gamma_u*(1-Gamma_u)
        dZc = dc_t*Gamma_u*self.activation_derivative(zc,activation="relu")
        dZo = da_t*self.relu(c_t)*Gamma_o*(1-Gamma_o)
